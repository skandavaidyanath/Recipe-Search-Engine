...
def db2text():
    '''
    Splits the data in out user_clicks table in the database to a training set and test set with a 4:1 ration to train our recommender system.
    '''
    db = get_db()
    cursor = db.cursor()
    fp1 = open('q1.test', 'w+')
    ...
    try:
        ...
        test_indices = random.sample(
            range(min_index, max_index+1), int(test_cases))
        for index in test_indices:
            try:
                ...
                docid = row[2]
                clicks = row[3]
                fp1.write(str(userid) + '\t' + str(docid) +
                          '\t' + str(clicks) + '\n')
        ...
        training_indices = list(
            filter((lambda x: x not in test_indices), range(min_index, max_index+1)))
        for index in training_indices:
            try:
                ...
                docid = row[2]
                clicks = row[3]
                fp2.write(str(userid) + '\t' + str(docid) +
                          '\t' + str(clicks) + '\n')
